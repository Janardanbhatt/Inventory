...
export default function EditComponent () {
	  ...
	  useEffect(()=>{
	  		fetch(`http://localhost:3000/posts/${window.location.pathname.split("/")[2]}`).then((response) => {
	...
	function handleSubmit(event) {
    	...
    	const form = event.currentTarget;
		    if (form.checkValidity() === false) {
		      ...
		      event.stopPropagation();
		    	...
		    	fetch(`http://localhost:3000/posts/${window.location.pathname.split("/")[2]}`,{
		    		method:'PUT',
		    		body : JSON.stringify(itemDetailsUpdate),
		    		headers: {
		 ...
		 setValidated(true);
			...
			return(
					<Form className='Edit' noValidate validated={validated} onSubmit={handleSubmit}>
